...
curl -X POST -H "Content-Type: application/json" -d '{"key": "value", "anotherKey": "anotherValue"}' http://localhost:3000/api/v1/ping

# load test
npx autocannon --renderStatusCodes http://localhost:3000/non-blocking

# cache middleware stuff
const myCache = require('node-cache'); // Or other caching library
const cache = new myCache({ stdTTL: 60 }); // Example with node-cache
...
const cacheMiddleware = (req, res, next) => {
    ...
    if (cachedData) {
        ...
        // Store original res.send/json to capture data before sending
...
# Workers
Use asynchronous operations (async/await) for:
Database interactions
API calls
File system operations
Any other task that involves waiting for an external resource.
